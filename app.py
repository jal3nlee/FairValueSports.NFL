...
def run_app():
    ...
    with tabs[3]:
        ...
        # ✅ Initial sanity test
        st.write("✅ Tab 4 loaded successfully — API call starting...")

        # --- Load NFL Teams (with fail-safe) ---
        def get_teams():
            try:
                ...
                st.error(f"Error during team fetch: {e}")
                return []

        # --- Load Players for Selected Team (with fail-safe) ---
        def get_team_players(team_id):
            try:
                url = f"{API_BASE}/players?team={team_id}&season=2025"
                r = requests.get(url, headers=HEADERS, timeout=10)
                if r.status_code != 200:
                    st.error(f"Failed to fetch players. Status: {r.status_code}")
                    st.write("Response:", r.text)
                    return []
                data = r.json().get("response", [])
                players = []
                ...
                st.error(f"Error during player fetch: {e}")
                return []
        ...
        if not teams:
            st.warning("No team data available — check API key or quota.")
        else:
